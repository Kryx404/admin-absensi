...
import { getLiburNasional } from "../api/libur";
...
const Calendar: React.FC = () => {
    ...
    useEffect(() => {
        // Initialize with some events and fetch Indonesian public holidays
        ...
        // Fetch Indonesian public holidays for current year
        const year = new Date().getFullYear();
        getLiburNasional(year)
            .then((holidays) => {
                const holidayEvents: CalendarEvent[] = holidays.map(
                    (h, idx) => ({
                        id: `libur-${h.date}`,
                        title: `Libur Nasional: ${h.localName}`,
                        start: h.date,
                        allDay: true,
    ...
    const isLiburNasional =
        selectedEvent && selectedEvent.id?.startsWith("libur-");
    const handleAddOrUpdateEvent = () => {
        if (isLiburNasional) return; // Prevent editing libur nasional
        if (selectedEvent) {
                        ...
                        <div>
                            ...
                            {isLiburNasional && (
                                <span className="inline-block mb-2 px-3 py-1 rounded-full bg-yellow-100 text-yellow-800 text-xs font-semibold border border-yellow-300 dark:bg-yellow-900/30 dark:text-yellow-200 dark:border-yellow-700">
                                    Event ini diambil dari data Libur Nasional
                                    Indonesia dan tidak dapat diedit.
                                </span>
                            )}
